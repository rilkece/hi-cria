...
import com.cria.agora.GamePerformance;
...
public class MyGamePerformanceAdapter extends RecyclerView.Adapter<MyGamePerformanceAdapter.MyViewHolder> {

    Context context;
    ArrayList<UserGamePerformance> perfilGameScore;

    public MyGamePerformanceAdapter(Context c, ArrayList<UserGamePerformance> p){
        context = c;
        perfilGameScore = p;
    ...
    public void onBindViewHolder(@NonNull final MyViewHolder myViewHolder, final int i) {
        String Name = perfilGameScore.get(i).getName()+" "+perfilGameScore.get(i).getSurname();
        String schoolName = perfilGameScore.get(i).getSchool();
        String pontuacao = String.valueOf(perfilGameScore.get(i).getgameScore());
        ...
        //mudar avatar
        DatabaseReference refAv = FirebaseDatabase.getInstance().getReference("users").child(perfilGameScore.get(i).getMat()).child("desempenho").child("avatar");
                ...
                myViewHolder.imgPerfProfile.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if (sharedPreferences.getLong("tipoUser", 0) ==0){
                            DatabaseReference refAv = FirebaseDatabase.getInstance()
                                    .getReference("users")
                                    .child(perfilGameScore.get(i).getMat())
                                    .child("desempenho")
                                    .child("games")
                                    .child(GamePerformance.stgNameGame);

                            //como ler data do último login
                            refAv.child("lastGameLogin").addListenerForSingleValueEvent(new ValueEventListener() {
                                @Override
                                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
    ...
    private String getDate(long time) {
        ...
        Toast.makeText(context.getApplicationContext(), GamePerformance.stgGame+" last login: "+date, Toast.LENGTH_LONG).show();
        return date;
    }
